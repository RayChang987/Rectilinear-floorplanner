...
void sequence_pair_enumerator_t::generate_sequence_pairs(int n) {
    ...
    if (sequence_pair_t::soft_n != 0) {
        ...
        bool success = this->seed_SP.find_position(true, true);
...
bool sequence_pair_enumerator_t::add_soft_process_cont(int i, bool with_area,
                                                       int cutoff, int start_j,
                                                       int start_k) {
    this->init_timer.timer_end();
    ...
    if (i >= this->seed_SP.fix_start_idx) {
        // this->seed_SP.predict_wirelength(true, false); //minimize wirelength
        // at last
        this->res_SP = this->seed_SP;
        // if(this->seed_SP.actual_wirelength!=-1){
        this->valid_sequence_pairs.push_back(this->seed_SP);
        //}
        this->current_sp_n++;
    ...
    if (this->seed_SP.is_in_seq[this->seed_SP.add_soft_order[i]]) {
        return add_soft_process_cont(i + 1, with_area, cutoff, start_j,
                                     start_k);
    }
    ...
    if (seed_SP.v_sequence.size() == 0) {  // in case there are no fix module
        ...
        bool success;
        if (with_area) {
            success = this->seed_SP.find_position_with_area(false, false);
        } else {
            success = this->seed_SP.find_position(false, false);
        }
    ...
    for (int j = 0; j <= seed_SP.v_sequence.size(); ++j) {
        for (int k = 0; k <= seed_SP.h_sequence.size(); ++k) {
            ...
            seed_SP.v_sequence.insert(seed_SP.v_sequence.begin() + jj,
                                      this->seed_SP.add_soft_order[i]);
            seed_SP.h_sequence.insert(seed_SP.h_sequence.begin() + kk,
                                      this->seed_SP.add_soft_order[i]);
            this->seed_SP.is_in_seq[this->seed_SP.add_soft_order[i]] = 1;
            bool success;
            if (with_area) {
                ...
                // success = this->seed_SP.find_position_allow_illegal(true,
                // true, 0, 0);
                success = this->seed_SP.find_position(true, true);
            }
            if (success) {
                double wl = this->seed_SP.lp_predicted_wirelength;
                // legal_pos.push_back({wl, {jj, kk}});
                // break;
                if (this->add_soft_process_cont(i + 1, with_area, cutoff, jj,
                                                kk)) {
                    return true;
                }
            }
...
void sequence_pair_enumerator_t::updated_best_SP() {
    ...
    double current_best_wirelength =
        this->valid_sequence_pairs[0].get_wirelength();
