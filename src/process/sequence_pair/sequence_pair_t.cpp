...
void sequence_pair_t::init() {
    const std::vector<soft_module_t*>& soft_modules =
    ...
    sequence_pair_t::sequence_n =
        static_cast<int>(chip_t::get_total_module_n());
    sequence_pair_t::fix_start_idx =
        static_cast<int>(chip_t::get_soft_modules().size());
...
sequence_pair_t::sequence_pair_t() {
    ...
    int j = 0;
    for (int i = 0; i < soft_modules.size(); ++i) {
        this->soft_area_to_w_h_m_5[i] =
            sequence_pair_t::find_w_h(soft_modules[i]->get_area(), 5);
        this->soft_area_to_w_h_m_9[i] =
            sequence_pair_t::find_w_h(soft_modules[i]->get_area(), 9);
        j++;
    }
    init_modules_size();
    set_is_in_seq(0);
...
std::vector<int> sequence_pair_t::get_v() { return this->v_sequence; }

std::vector<int> sequence_pair_t::get_h() { return this->h_sequence; }
...
vector<vec2d_t> sequence_pair_t::find_w_h(uint32_t area, int resol) {
    ...
    double b = pow(fp_rule_t::get_upper_ratio(),
                   1.00 / static_cast<double>(resol - 1));
    vector<vec2d_t> res;
    ...
    for (int i = 0; i <= resol - 1; ++i) {  // i will be the shorter edge
        ...
        if (ceil(x) * fp_rule_t::get_upper_ratio() >= ceil(y)) {
            ...
            if (xx - 1 >= yy * fp_rule_t::get_lower_ratio() &&
                (xx - 1) * yy >= area) {
                xx -= 1;
            }
            if (yy - 1 >= fp_rule_t::get_lower_ratio() * xx &&
                (yy - 1) * xx >= area) {
                yy -= 1;
            }
...
void sequence_pair_t::build_constraint_graph() {
    ...
    for (int i = 0; i < sequence_n; ++i) {
        for (int j = 0; j < sequence_n; ++j) {
            if (is_in_seq[i] == 0 || is_in_seq[j] == 0) {
                continue;
            }
            if (v_map[i] < v_map[j] && h_map[i] < h_map[j]) {  //{a, b}, {a, b}
                this->constraint_graph_h.push_back(
                    {i, j, static_cast<int>(this->modules_wh[i].get_x())});
            } else if (v_map[i] < v_map[j] &&
                       h_map[j] < h_map[i]) {  //{i, j}, {j,i}
                this->constraint_graph_v.push_back(
                    {i, j, static_cast<int>(this->modules_wh[i].get_y())});
            }
        }
    }
}
    ...
    for (int i = 0; i < sequence_n; ++i) {
        if (this->is_in_seq[i] == 0) {
            ...
            result_pos.push_back(
                {static_cast<int>(ILP_result.var_values[i + x_module_offset]),
...
bool sequence_pair_t::find_position(bool minimize_wirelength,
    ...
    constraint_n = this->constraint_graph_h.size() +
                   ...
                   chip_t::get_fixed_modules().size() * 2 + 5 * soft_n;
    ...
    constraint_n += 2 * sequence_pair_t::connections.size() +
                    4 * sequence_pair_t::connection_graph_deg;
    x_module_offset = 1;
    ...
    x_edge_offset_r =
        1 + 2 * sequence_n + 5 * soft_n + this->connections.size();
    y_edge_offset_l =
        1 + 2 * sequence_n + 5 * soft_n + 2 * this->connections.size();
    y_edge_offset_r =
        1 + 2 * sequence_n + 5 * soft_n + 3 * this->connections.size();
        ...
        if (load_result) {
            ...
            for (int i = 0; i < sequence_pair_t::sequence_n; ++i) {
                if (seq_is_fix[i] == false) {
                    this->modules_wh[i] = {
                        static_cast<int>(result_wh[i].get_x()),
                        static_cast<int>(result_wh[i].get_y())};
                }
                this->modules_positions[i] = {
                    static_cast<int>(result_pos[i].get_x()),
                    static_cast<int>(result_pos[i].get_y())};
            }
...
void sequence_pair_t::sequence_pair_validation() {
    ...
    for (int i = 0; i < sequence_pair_t::sequence_n; ++i) {
        if (this->modules_positions[i].get_x() < 0 ||
            this->modules_positions[i].get_y() < 0) {
            continue;
        }
...
void sequence_pair_t::set_bounding_lines() {
    ...
    for (int i = 0; i < sequence_pair_t::sequence_n; ++i) {
        if (this->carved[i]) {
            ...
            this->bounding_lines[i] = {
                this->get_4_points(this->modules_positions[i],
                                   this->modules_wh[i]),
...
void sequence_pair_t::build_graph() {
    ...
    for (multi_net_t* input_multi_net : input_multi_nets) {
        ...
        sequence_pair_t::connections.push_back(
            {connected_id, input_multi_net->weight});
    }
...
bool sequence_pair_t::check_area_ratio() {
    ...
    // this function will check the area, 1:2 constraints, but no overlap and
    // 80% constraint
    for (int i = 0; i < sequence_pair_t::sequence_n; ++i) {
        if (sequence_pair_t::seq_is_fix[i]) {
            continue;
        ...
        double ratio =
            this->modules_wh[i].get_x() / this->modules_wh[i].get_y();
...
void sequence_pair_t::set_vi(int i, int x) { this->v_sequence[i] = x; }

void sequence_pair_t::set_hi(int i, int x) { this->h_sequence[i] = x; }

int sequence_pair_t::get_vi(int i) { return this->v_sequence[i]; }

int sequence_pair_t::get_hi(int i) { return this->h_sequence[i]; }

void sequence_pair_t::change_size(int i) {
    if (sequence_pair_t::seq_is_fix[i]) {
        this->modules_wh[i] = sequence_pair_t::seq_fixed_map[i]->get_size();
    } else {
        int id = static_cast<int>(random_helper::get_rand()) %
                 sequence_pair_t::soft_area_to_w_h_m_5[i].size();
...
void sequence_pair_t::set_fix_sequence() {
    ...
    vector<vector<int>> fix_constraint_v(
        sequence_pair_t::sequence_n,
        vector<int>(sequence_pair_t::sequence_n, 0));
    vector<vector<int>> fix_constraint_h(
        sequence_pair_t::sequence_n,
        vector<int>(sequence_pair_t::sequence_n, 0));
    for (int i = sequence_pair_t::fix_start_idx;
         i < sequence_pair_t::sequence_n; ++i) {
        for (int j = sequence_pair_t::fix_start_idx;
             j < sequence_pair_t::sequence_n; ++j) {
            if (i == j) {
                continue;
            }
    ...
    vector<int> upd_v, upd_h, v_map(sequence_pair_t::sequence_n),
        h_map(sequence_pair_t::sequence_n);
    for (int i = sequence_pair_t::fix_start_idx;
         i < sequence_pair_t::sequence_n; ++i) {
        ...
        for (int j = 0; j <= upd_v.size() && !inserted; ++j) {
            for (int k = 0; k <= upd_h.size() && !inserted; ++k) {
                bool ok = true;
                for (int p = sequence_pair_t::fix_start_idx; p < i;
                     ++p) {  // check all elements in squence
                    int p_id_v = v_map[p], p_id_h = h_map[p];
                    if (j <= p_id_v &&
                        k <= p_id_h) {  // V[i, p], H[i, p] then need i->p in
                                        // horizontal constraints
                        if (fix_constraint_h[i][p]) {
                            continue;
                        } else {
                            ok = false;
                            break;
                    ...
                    if (j > p_id_v &&
                        k > p_id_h) {  // V[p, i], H[p, i] then need p->i in
                                       // horizontal constraints
                        if (fix_constraint_h[p][i]) {
                            continue;
                        } else {
                            ok = false;
                            break;
                    ...
                    if (j <= p_id_v &&
                        k > p_id_h) {  // V[i, p], H[p, i] then need i->p in
                                       // vertical constraints
                        if (fix_constraint_v[i][p]) {
                            continue;
                        } else {
                            ok = false;
                            break;
                    ...
                    if (j > p_id_v &&
                        k <= p_id_h) {  // V[p,i], H[i, p] then need p->i in
                                        // vertical constraints
                        if (fix_constraint_v[p][i]) {
                            continue;
                        } else {
                            ok = false;
                            break;
...
void sequence_pair_t::print_result() {
    ...
    for (int i = 0; i < sequence_pair_t::sequence_n; ++i) {
        double area = this->modules_wh[i].get_x() * this->modules_wh[i].get_y();
        cout << i << ", pos : " << std::setprecision(16)
             << this->modules_positions[i] << endl;
        cout << i << ", area: " << std::setprecision(16) << area << endl;
        cout << i << ", wh : " << std::setprecision(16) << this->modules_wh[i]
             << endl;
        cout << i << ", minimum area: " << std::setprecision(16)
             << this->modules_area[i] << endl;
        cout << i << ", area - minimum: " << std::setprecision(16)
             << area - this->modules_area[i] << endl;
        cout << i << ", area - compensation: " << std::setprecision(16)
             << area - area_compensation[i] << endl;
        cout << i << ", Ratio: " << std::setprecision(3)
             << (1 - static_cast<double>(area_compensation[i]) /
                         this->modules_area[i]) *
                    100
             << "%" << endl;
    }
...
void sequence_pair_t::save_result_checker() {
    ...
    for (int i = 0; i < soft; ++i) {
        ...
        double ratio = bd.get_bounding_rect().get_size().get_x() /
                       bd.get_bounding_rect().get_size().get_y();
        double percent = bd.get_area() / bd.get_bounding_rect().get_area();
        int ver = bd.get_vertex_count();
        total_vertexes += ver;
        int ed = bd.get_edge_count();
        total_edges += ed;
...
void sequence_pair_t::set_add_order() {
    ...
    std::sort(this->add_soft_order.begin(), this->add_soft_order.end(),
              [&](int& a, int& b) {
...
void sequence_pair_t::print_logs() {
    for (auto& log : this->logs) {
        cout << "After " << log.first << "s, got wire length" << log.second
             << endl;
    }
}
...
bool sequence_pair_t::find_position_with_area(bool minimize_wirelength,
    ...
    constraint_n = this->constraint_graph_h.size() +
                   this->constraint_graph_v.size() +
                   chip_t::get_fixed_modules().size() * 2;
    constraint_i = 1;  // constraint_counter
    variable_n = 2 * sequence_n;
    if (minimize_wirelength) {
        variable_n += 4 * sequence_pair_t::connections.size();
        constraint_n += (2 + 4 * sequence_pair_t::connection_graph_deg) *
                        sequence_pair_t::connections.size();
    }
    x_module_offset = 1;
    y_module_offset = 1 + sequence_n;
    ...
    for (int i = 1; i <= sequence_n; ++i) {
        if (this->is_in_seq[i - 1] == 0) {
            ...
            result.push_back({(ILP_result.var_values[i]),
                              (ILP_result.var_values[i + sequence_n])});
        }
    }
    if (load_result) {
...
bool sequence_pair_t::find_position_allow_illegal_fill(bool minimize_wirelength,
    ...
    simplify_constraint_graph();
    constraint_n = 3 * this->constraint_graph_h.size() +
                   3 * this->constraint_graph_v.size() +
                   chip_t::get_fixed_modules().size() * 2 + 5 * soft_n +
                   2 * sequence_pair_t::sequence_n;
    constraint_i = 1;  // constraint_counter
    variable_n = 2 * sequence_n + 5 * soft_n +
                 2 * this->constraint_graph_h.size() +
                 this->constraint_graph_v.size();
    if (minimize_wirelength) {
        variable_n += 4 * sequence_pair_t::connections.size();
        constraint_n += 2 * sequence_pair_t::connections.size() +
    ...
    y_overlap = 1 + 2 * sequence_n + 5 * soft_n + 4 * this->connections.size() +
                this->constraint_graph_h.size();
    ...
    if (ILP_result.legal) {
        if (load_result) {
            ...
            this->modules_positions.resize(result_pos.size());
            for (int i = 0; i < sequence_pair_t::soft_n; ++i) {
                this->modules_wh[i] = {static_cast<int>(result_wh[i].get_x()),
                                       static_cast<int>(result_wh[i].get_y())};
            }
            this->modules_wh_i = result_wh_i;
            for (int i = 0; i < sequence_pair_t::sequence_n; ++i) {
                // cout<< i<<" "<<seq_is_fix[i]<<",
                // "<<"{"<<ILP_result.var_values[i+this->x_overlap]<<",
                // "<<ILP_result.var_values[i+this->y_overlap]<<"}"<<endl;
                this->modules_positions[i] = {
                    static_cast<int>(result_pos[i].get_x()),
                    static_cast<int>(result_pos[i].get_y())};
                if (ILP_result.var_values[i + this->x_overlap] ||
                    ILP_result.var_values[i + this->y_overlap]) {
...
bool sequence_pair_t::find_position_allow_illegal(bool minimize_wirelength,
    ...
    for (int i = 0; i < sequence_n; ++i) {
        new_shape_5.push_back(sequence_pair_t::find_w_h(
            this->modules_area[i] + area_compensation[i], 5));
    }
    // cout<<endl;
...
void sequence_pair_t::fill_near() {
    this->near_x.clear();
    this->near_y.clear();
    this->near_x_map.clear();
    this->near_y_map.clear();
    this->near_y_id.clear();
    this->near_x_id.clear();

    this->near_x_map.resize(this->sequence_n);
    this->near_y_map.resize(this->sequence_n);

    near_y_id = near_x_id = vector<vector<int>>(sequence_pair_t::sequence_n,
                                                vector<int>(sequence_n, -1));
    int ix, iy;
    ix = iy = 0;
    ...
    for (int i = 0; i < this->sequence_n; ++i) {
        ...
        for (int j = 0; j < this->sequence_n; ++j) {
            if (i == j || seq_is_fix[i] || seq_is_fix[j]) {
                continue;
            }
            if (area_comp[j]) {
                continue;
            }
            if ((modules_positions[i] + modules_wh[i]).get_x() ==
                modules_positions[j].get_x()) {
                int lower_y_i = modules_positions[i].get_y();
                int upper_y_i =
                    modules_positions[i].get_y() + modules_wh[i].get_y();
                int lower_y_j = modules_positions[j].get_y();
                int upper_y_j =
                    modules_positions[j].get_y() + modules_wh[j].get_y();
                int lower = std::min(lower_y_i, lower_y_j);
                int upper = std::max(upper_y_i, upper_y_j);
                if ((lower_y_i <= upper_y_j && lower_y_i >= lower_y_j) ||
                    (upper_y_i <= upper_y_j && upper_y_i >= lower_y_j)) {
                    // i -> j

                    int ih = std::min(
                        modules_wh[i].get_x() / 4.01,
                        1.949 * modules_wh[i].get_y() - modules_wh[i].get_x());
                    int jh = std::min(
                        modules_wh[j].get_x() / 4.01,
    ...
    for (int i = 0; i < this->sequence_n; ++i) {
        ...
        for (int j = 0; j < this->sequence_n; ++j) {
            ...
            if ((modules_positions[i] + modules_wh[i]).get_y() ==
                modules_positions[j].get_y()) {
                int lower_x_i = modules_positions[i].get_x();
                int upper_x_i =
                    modules_positions[i].get_x() + modules_wh[i].get_x();
                int lower_x_j = modules_positions[j].get_x();
                int upper_x_j =
                    modules_positions[j].get_x() + modules_wh[j].get_x();
                int lower = std::min(lower_x_i, lower_x_j);
                int upper = std::max(upper_x_i, upper_x_j);
                if ((lower_x_i <= upper_x_j && lower_x_i >= lower_x_j) ||
                    (upper_x_i <= upper_x_j && upper_x_i >= lower_x_j)) {
                    // i -> j

                    int ih = std::min(
                        modules_wh[i].get_y() / 4.01,
                        1.949 * modules_wh[i].get_x() - modules_wh[i].get_y());
                    int jh = std::min(
                        modules_wh[j].get_y() / 4.01,
...
void sequence_pair_t::overlap_optimization() {
    constraint_n = 10 * sequence_pair_t::connections.size() +
                   2 * sequence_pair_t::sequence_n + 2 * this->near_x.size() +
                   2 * this->near_y.size();
    variable_n = 2 * sequence_n + 4 * connections.size() + this->near_x.size() +
                 this->near_y.size();
    constraint_i = 1;  // constraint_counter
    ...
    y_edge_offset_l =
        1 + 2 * sequence_n + 2 * this->connections.size();  // e_x_l
    y_edge_offset_r =
        1 + 2 * sequence_n + 3 * this->connections.size();  // e_x_r
    near_x_offset = 1 + 2 * sequence_n + 4 * this->connections.size();
    near_y_offset =
        1 + 2 * sequence_n + 4 * this->connections.size() + this->near_x.size();
    ...
    // cout<<"Z after rectilinear optimization:
    // "<<std::setprecision(16)<<ILP_result.z<<endl;
...
void sequence_pair_t::deal_bounding_line() {
    ...
    for (int i = 0; i < n; ++i) {
        ori_bounding_lines.push_back(
            bounding_line_t(this->bounding_lines[i].first));
        ori_neg_bounding_lines.push_back(
            bounding_line_t(this->bounding_lines[i].first, false));
    }
    ...
    for (int i = 0; i < soft; ++i) {
        for (int j = i + 1; j < soft; ++j) {
            if (ori_bounding_lines[i].collision(ori_neg_bounding_lines[j])) {
                if (ori_bounding_lines[i].get_area() <
                    ori_bounding_lines[j].get_area()) {
    ...
    for (int i = 0; i < soft; ++i) {
        bounding_line_t bd0 = bounding_line_t(this->bounding_lines[i].first);
        for (int j = soft; j < n; ++j) {
            bounding_line_t bd1 =
                bounding_line_t(this->bounding_lines[j].first, false);
            if (!bd0.collision(bd1)) {
                continue;
            }
            auto bds = bounding_line_t::merge(bd0, bd1);
            if (!bds.difference_pos_line.empty())
                bd0 = bds.difference_pos_line[0];
        }
        for (auto num : collision_num[i]) {
            if (i == num) continue;
            bounding_line_t bd1 =
                bounding_line_t(this->bounding_lines[num].first, false);
            auto bds = bounding_line_t::merge(bd0, bd1);
            if (!bds.difference_pos_line.empty())
                bd0 = bds.difference_pos_line[0];
        }
        this->bounding_lines[i] = {bd0.get_nodes(),
                                   this->bounding_lines[i].second};
    }
}

double sequence_pair_t::get_wirelength() {
    ...
    for (int i = 0; i < connections.size(); ++i) {
        ...
        for (int j = 0; j < connections[i].nodes.size(); ++j) {
            ...
            vec2d_t center_v = {
                ...
                pos[v].get_y() + this->modules_wh[v].get_half_y()};
...
double sequence_pair_t::get_wirelength_rectilinear() {
    ...
    double sum = 0;
    for (int i = 0; i < sequence_pair_t::sequence_n; ++i) {
        vec2d_t center = {this->modules_positions[i].get_x() +
                              this->modules_wh[i].get_half_x(),
                          this->modules_positions[i].get_y() +
                              this->modules_wh[i].get_half_y()};
        centers.push_back(center);
    }
    for (int i = 0; i < result_carving_x.size(); ++i) {
        if (result_carving_x_enable[i] == false) {
            continue;
        }
    ...
    for (int i = 0; i < result_carving_y.size(); ++i) {
        if (result_carving_y_enable[i] == false) {
            continue;
        }
...
void sequence_pair_t::print_wirelength() {
    cout << std::setprecision(16) << this->get_wirelength() << endl;
}
...
void sequence_pair_t::simplify_constraint_graph() {
    ...
    this->is_transitive_v = this->is_transitive_h =
        vector<vector<int>>(sequence_pair_t::sequence_n,
                            vector<int>(sequence_pair_t::sequence_n, 0));
    {
    ...
    for (int i = 0; i < soft; ++i) {
        for (int j = i + 1; j < n; ++j) {
            if (rects[i].is_collision(rects[j]) &&
                this->modules_area[i] > this->modules_area[j])
