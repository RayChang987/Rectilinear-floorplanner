...
fixed_module_t* fixed_module_t::fileInput(std::fstream& file) {
    ...
    file >> fixedModule->name >> fixedModule->xCoord >> fixedModule->yCoord >>
        fixedModule->width >> fixedModule->height;
    fixedModule->rect =
        new rect_t(vec2d_t(fixedModule->xCoord, fixedModule->yCoord),
...
const uint32_t fixed_module_t::get_area() const {
    return this->rect == nullptr
               ? 0
               : static_cast<const uint32_t>(this->rect->get_area());
}
...
bounding_rectangle_t fixed_module_t::make_bd() { return {this, *this->rect}; }

std::pair<bounding_rectangle_t, bool> fixed_module_t::make_bd(
